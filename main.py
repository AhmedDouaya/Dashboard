...
with open("pred_frame_s1000.pkl", "rb") as pickle_out:
    pred_frame_s1000 = pickle.load(pickle_out)
...
def requet_ID(ID):
    ...
    if ID_client not in list(pred_frame_s1000['SK_ID_CURR']):
        result = 'Ce client n\'est pas dans la base de données.'

    else:

        # Récupération des infos clients
        X_ID = pred_frame_s1000[pred_frame_s1000['SK_ID_CURR'] == ID_client].copy()
        ...
        if y_Target.item() == 0:
            result = ('Ce client est solvable avec un taux de risque de ' + str(round(y_proba.iloc[0] * 100, 2)) + '%')
...
def comparaison_client_voisin(ID, frame_shap, frame_hist):
    ...
    st.write('Le client', str(ID_c), 'à un écart de', str(diff_proba),
             '% de risque avec les clients de profils similaires.')
